...
public class LinkbackExtractor
{
    private static Log mLogger = 
        LogFactory.getFactory().getInstance(LinkbackExtractor.class);
    	...
    	throws MalformedURLException, IOException, ParseException 
	{
        //FlockFeedFactory factory = new FlockFeedFactory();
 		//FlockFeedI flockFeed = factory.createFeed(new URL(rssLink));
        //Iterator articleIter = flockFeed.getArticles().iterator();
        //String feedTitle = flockFeed.getTitle();
        ChannelIF channel = RSSParser.parse(
            new ChannelBuilder(), new URL(rssLink));
        ...
        {
            count++;
            //FlockArticleI item = (FlockArticleI)itemIter();
    ...
    {    
        ...
        // Dividers
        private Tag[] mDivTags =
        {
            Tag.TD, Tag.DIV, Tag.SPAN, Tag.BLOCKQUOTE, Tag.P, Tag.LI, Tag.BR, 
            Tag.HR, Tag.PRE, Tag.H1, Tag.H2, Tag.H3, Tag.H4, Tag.H5, Tag.H6                        
        };
        ...
        public void handleStartTag(
            Tag tag, MutableAttributeSet atts, int pos)
        {
            ...
            else if ( tag.equals(Tag.A) )
            {
                ...
                if ( href != null && 
                   ( href.equals(mRequestURL) || href.equals(mRequestURLWWW)))
                {
                    mFound = true;   
                }
                else
        ...
        public void handleSimpleTag(Tag tag, MutableAttributeSet atts, int pos)
        {
            ...
            else if ( tag.equals(Tag.LINK) )
            {
            	...
            	if ( title!=null && type!=null 
                  && type.equals("application/rss+xml") && title.equals("RSS"))
            	{
                    ...
                    if ( mRssLink.startsWith("/") && mRssLink.length()>1 )
                    {
                        try
                        {
                            ...
                            mRssLink = url.getProtocol()+"://"+url.getHost()+
                                ":"+url.getPort()+mRssLink;
                        }
