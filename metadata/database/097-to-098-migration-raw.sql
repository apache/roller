...
CREATE table rolleruser ( 
    ...
    userenabled     @BOOLEAN_SQL_TYPE2@ not null
);
INSERT INTO rolleruser 
    (id, username, passphrase, fullname, emailaddress, datecreated) 
    SELECT tempuser.id, username, password, fullname, emailaddress, e1.pubtime
        FROM tempuser, website, weblogentry e1, weblogentry e2
			where tempuser.id = website.userid
			and website.id = e1.websiteid
			and e2.websiteid = e1.websiteid
			and e1.id < e2.id
			group by website.userid
			order by e1.pubtime;
