...
-- add new columns which hold the multi-lang settings for a weblog
    alter table website add column enablemultilang smallint with default 0 not null;
    alter table website add column showalllangs smallint with default 1 not null;

-- add new column which holds the hidden status for a page, default is false
    alter table webpage add column hidden smallint with default 0 not null;

-- add new column which holds the hidden status for a page, default is false
    alter table webpage add column navbar smallint with default 0 not null;
