...
alter table roller_user_permissions add constraint up_userid_fk
    foreign key ( user_id ) references rolleruser( id ) on delete no action on update no action enforced enable query optimization ;

alter table roller_user_permissions add constraint up_websiteid_fk
    foreign key ( website_id ) references website( id ) on delete no action on update no action enforced enable query optimization ;
