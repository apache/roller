...
-- add new field to support comment plugins and content-type
    ...
    alter table roller_comment add column contenttype varchar(128) with default 'text/plain' not null;

-- add new status field to comment table to simplify queries
    alter table roller_comment add column status varchar(20) with default 'APPROVED' not null;
...
-- new column to support screen name and populate with user names
    alter table rolleruser add column screenname varchar(255) with default 'unspecified' not null;
...
-- add new action column to webpage table, default value is custom
    alter table webpage add column action varchar(16) with default 'custom' not null;
...
-- You should drop table rollerconfig if it exists

-- remove old id column of group subscription table
-- -- You should alter table rag_group_subscription drop column id;

-- remove old approved, spam, pending columns from comment table
-- You should alter table roller_comment drop column approved;
-- You should alter table roller_comment drop column spam;
-- You should alter table roller_comment drop column pending;

-- remove bastard columns and indexes (optional)
-- drop index index_we_pubtime_idx;
-- drop index co_pending_idx;
-- drop index co_approved_idx;
-- -- You should alter table website drop column userid;
-- -- You should alter table website drop column weblogdayid;
-- -- You should alter table weblogentry drop column publishentry;
-- -- You should alter table weblogentry drop column link;
-- -- You should drop table usercookie if it exists
-- -- You should drop table rag_config if it exists
-- -- You should drop table folderassoc if it exists
-- -- You should drop table weblogcategoryassoc if it exists
